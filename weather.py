...
def fetch_weather(lat, lon):
    r=requests.get(
        ...
        params={"latitude": lat, "longitude": lon, "current": ["temperature_2m", "apparent_temperature", "relative_humidity_2m", "wind_speed_10m", "wind_direction_10m", "precipitation", "weathercode", "uv_index"], "hourly": ["temperature_2m", "precipitation_probability"], "daily": ["temperature_2m_max", "temperature_2m_min", "precipitation_sum"], "timezone": "auto", "forecast_days": 7,}, timeout=8,
    )
...
st.caption("Get the current weather and 7-day for forecast for any city In the world.")
...
chosen_ind=st.selectbox("Select the correct locations", range(len(options)), format_func=lambda x: options[x])
